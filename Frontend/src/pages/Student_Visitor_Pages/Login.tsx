...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Capture the user data payload returned by the service[cite: 10, 12]
      const userData = await authService.login(credentials);
      
      // 2. REDIRECTION PROTOCOL: Inspect user_type from the database
      // Matches the AdminService logic we just built
      if (userData.userType === "Admin") {
        console.log("Admin Privileges Detected. Accessing Registry...");
        navigate("/Admin/UserManagement"); 
      } else {
        console.log("Standard Access Granted. Accessing Dashboard...");
        navigate("/Dashboard");
      }
      ...
      // Verification logic maintained for unverified accounts[cite: 12]
      ...
      setIsLoading(false); // Reset button state[cite: 12]
    }
  };
        ...
        <div className="bg-white w-full max-w-md mx-auto lg:ml-auto p-10 md:p-14 rounded-manuscript shadow-2xl relative overflow-hidden border border-orange-50">
          ...
          <form onSubmit={handleLogin} className="space-y-8">
            <div className={inputWrapper}>
              ...
              <input required type="email" className={inputClass} placeholder="name@email.com" value={credentials.email} onChange={(e) => setCredentials({...credentials, email: e.target.value})} />
            </div>

            <div className={inputWrapper}>
              <label className={labelClass}>PASSWORD</label>
              <input required type="password" className={inputClass} placeholder="••••••••" value={credentials.password} onChange={(e) => setCredentials({...credentials, password: e.target.value})} />
            </div>
